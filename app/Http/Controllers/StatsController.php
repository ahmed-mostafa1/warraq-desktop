...
class StatsController extends Controller
{
    ...
    protected function buildStats(): array
    {
        ...
        $byReligion = $this->religionBreakdown();
    ...
    protected function religionBreakdown(): array
    {
        $counts = [
            'muslim' => 0,
            'christian' => 0,
        ...
        Member::query()
            ...
            ->get()
            ->each(function ($row) use (&$counts): void {
                $key = $this->normalizeReligionKey($row->religion);
                $counts[$key] += (int) $row->count;
            });

        return [
            ['key' => 'muslim', 'count' => $counts['muslim']],
            ['key' => 'christian', 'count' => $counts['christian']],
            ['key' => 'unspecified', 'count' => $counts['unspecified']],
        ];
    }
    ...
    protected function normalizeReligionKey(mixed $value): string
    {
        if ($value === null) {
            return 'unspecified';
        }
